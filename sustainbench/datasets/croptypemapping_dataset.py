...
class CropTypeMappingDataset(SustainBenchDataset):
    ...
    def __init__(self, version=None, root_dir='data', download=False,
                 split_scheme='official', splitting_technique='regionwise',
        ...
        if self._split_scheme in ['official', 'ghana', 'southsudan']:
            ...
            split_df = pd.read_csv(os.path.join(self.data_dir, self._country, f'cauvery_dataset_{splitting_technique}.csv'))
            ...
            split_df['y'] = split_df['UNIQUE_ID']
            split_df['partition'] = split_df['SPLIT'].apply(lambda split: partition_to_idx[split])
            self.split_plots = [split_df[split_df['SPLIT'] == 'train']['PLOT_ID'].astype(np.int).unique(),
                            split_df[split_df['SPLIT'] == 'val']['PLOT_ID'].astype(np.int).unique(),
                            split_df[split_df['SPLIT'] == 'test']['PLOT_ID'].astype(np.int).unique()]
        ...
        self._metadata_fields = ['y', 'PLOT_ID', 'partition']
    ...
    def get_input(self, idx):
        ...
        if self.resize_planet:
            ...
            # planet = planet[:, :, None, :,:]
            # # planet= transforms.ColorJitter(brightness=0.6, contrast=0, saturation=0, hue=0)(planet)
        ...
        return {'s1': s1[self.s1_bands], 's2': s2[self.s2_bands], 'l8': l8[self.l8_bands]}
    ...
    def get_label(self, idx):
        ...
        if self._split_scheme in ['official', 'ghana', 'southsudan']:
            ...
            split = self.y_array[idx][2].item()
            label = np.load(os.path.join(self.data_dir, self.country, f'truth@{self.truth_mask}m', f'{self.country}_{loc_id}.npz'))
            plot_id = label['plot_id'].astype(np.int)
            # print(plot_id)
            mask = np.zeros(plot_id.shape).astype(np.int)
            # print(self.split_plots[split])
            for i in range(mask.shape[0]):
                for j in range(mask.shape[1]):
                    if plot_id[i][j] in self.split_plots[split]:
                        mask[i][j] = 1
            # print(mask)
            # print(label['crop_type'])
            label = torch.from_numpy(np.expand_dims(label['crop_type']*mask, 0))
