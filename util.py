...
def split_with_group(df, group, train_frac, test_frac, data_cols, lbl_cols, random_seed=None, shuffle=True, save=False):
    ...
    train_inds, test_inds = next(GroupShuffleSplit(n_splits=3, test_size=test_frac,
                                                   train_size=train_frac, random_state=random_seed).split(X,
                                                                                                          groups=groups))
...
def mask_tif_npy(home, country, csv_source, crop_dict_dir, raster_dir):
    ...
    clustered_geom_id = [np.array(crop_csv['geom_id'][crop_csv['crop'] == crop_name]) for crop_name in
                         crop_dict.item().values()]
    ...
    for mask_fname in mask_fnames:
        with rasterio.open(os.path.join(mask_dir, mask_fname)) as src:
            ...
            for geom_id in mask_array_geom_id:
                if geom_id > 0:
                    crop_num = \
                    np.where([geom_id in clustered_geom_id[i] for i in np.arange(len(clustered_geom_id))])[0][0]
...
def get_num_bands(kwargs):
    ...
    if kwargs.get('include_clouds') and kwargs.get('use_s2'):
        added_clouds = 1
    if kwargs.get('include_indices') and (kwargs.get('use_s2') or kwargs.get('use_l8') or kwargs.get('use_planet')):
        added_indices = 2
